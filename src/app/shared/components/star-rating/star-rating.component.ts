...
export class StarRatingComponent implements OnInit, OnDestroy {
  ...
  setHoverRating(rating: number) {
    this.hoverRating = Math.round(rating * 2) / 2;
  }
  ...
  onStarHover(event: MouseEvent, clickedStar: EditableStar): void {
    ...
    this.hoverRating = clickedStar.position + ((!this.wholeStars && clickedInFirstHalf) ? 0.5 : 1);
  }
  ...
  private getStarClass(starType: StarType) {
    ...
    if (starType === 'empty')
      ...
      return 'ms ms-star-half'
    else
      return 'ms ms-star ms-fill'
  }
