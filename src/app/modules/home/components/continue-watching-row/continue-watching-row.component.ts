...
@Component({
  ...
  // DestroyService is component-scoped and required by the composed HistoryCardComponent.
  providers: [HistoryService, DestroyService, { provide: TRANSLOCO_SCOPE, useValue: ['media', 'home'] }],
  imports: [TranslocoDirective, HistoryCardComponent, TimeLeftPipe]
})
export class ContinueWatchingRowComponent implements OnInit {
  loading = signal(true);
  results = signal<HistoryGroupable[]>([]);
  // Placeholder cells reserve one card row while loading (matches the xl column count)
  // so cards swap in with no layout shift; CLS guardrail on the LCP-sensitive home.
  readonly skeletonCells = Array.from({ length: 6 });
